...
All IDs are deterministic. Use `kg-id.py` to generate — never invent ad-hoc.

- **entity_id**: `<domain>:<kebab-case-name>` (e.g., `billing:invoice-generation`).
  Generated by `kg-id.py entity <domain> <name>`.
- **fact_id**: `f_` + 8-char SHA-256 hash of `subject|predicate|object`.
  Generated by `kg-id.py fact <S> <P> <O>`.
- **contradiction_id**: `c_` + 8-char SHA-256 hash.
  Generated by `kg-id.py contradiction <quote1> <quote2>`.
- **question_id**: `oq_` + 8-char SHA-256 hash.
  Generated by `kg-id.py question <domain> <text>`.
...
Entity IDs use kebab-case for filenames: `billing:invoice-generation` →
`.magellan/domains/billing/entities/invoice-generation.md`.
Use `kg-id.py slug <text>` to generate safe filenames.
...
**Path**: `.magellan/domains/<domain>/entities/<entity-name>.md`

Entities use markdown with YAML frontmatter. The filename IS the entity slug
(kebab-case). Validate with `kg-id.py validate-entity <file>`.
...
tags: [business-rule]
edges:
  - to: billing:manual-review-bypass
    type: ENFORCES
    confidence: 0.95
---

Four-state invoice lifecycle (DRAFT → ISSUED → PAID) with a MANUAL_REVIEW
bypass for invoices exceeding $10k...

## Evidence

- **Q3_ops_runbook.pdf** p12: "Invoices exceeding $10,000 are routed to MANUAL_REVIEW..."
```

**Legacy note**: Older KGs may use `.json` entity files. The `kg-write.js add-entity`
tool still produces JSON. New KGs should use markdown+YAML and `kg-id.py validate-entity`.
**Required frontmatter**: `type`, `domain`, `confidence` (0.0–1.0), `weight` (0.0–1.0).

**Optional frontmatter**: `tags` (array), `edges` (array of `{to, type, confidence}`).

**Body**: narrative summary (min 50 chars) + `## Evidence` section with sourced quotes.
...
**Edge types**: `DEPENDS_ON`, `CALLS`, `READS_FROM`, `WRITES_TO`, `INTEGRATES_WITH`,
`ENFORCES`, `CONTAINS`, `TRIGGERS`, `PRODUCES`, `CONSUMES`, `PART_OF`, `SUCCEEDED_BY`,
`SAME_AS`, `SHARES_DATA_WITH`.

**Rules**:
- Each entity is self-contained. A reader with just this file has everything needed.
- Write entities one at a time, immediately after building.
- The body narrative is the most important — models read it first.
- Never delete entities — mark as `status: superseded` in frontmatter.
